...
module LogCheck
  ...
  RSpec.describe StrictValidator, '.ip_valid?' do
    subject { StrictValidator }
    ...
    it 'rejects octets greater than 255' do
      expect(subject.ip_valid?(invalid_ip_address_1)).to be_falsey
    end

    it 'rejects octets with leading 0s' do
      expect(subject.ip_valid?(invalid_ip_address_2)).to be_falsey
    end
